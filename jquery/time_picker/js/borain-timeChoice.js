 ...
 * 2018年2月11日 修复了147行和155行处对jq对象错误判断的bug
 */
//封装插件
...
function notEndNewTimeDateEvent(obj,level,less){
    ...
    //首先清除Class
    box.removeClass("YM YMD HM H HMS");
    //然后加上Class
    if(level==""||level=="YMD"){
    ...
    //插入对应值到控件
    timeChoiceValue(year,month,day,hour,minute,second);
    //显示控件
...
function choiceMonthSpanClick(year,less){
    ...
    choiceBox.find(".month").find("span").not(".not").unbind("click").click(function () {
        ...
        if(box.hasClass("YM")){
            timeChoiceValue(year,month,1,0,0,0);
        }else {
            //非年月选择到达月选择后都要进行天选择
            getDaysInMonth(year,month,less);
            ...
            if(box.hasClass("YMD")){
                timeChoiceValue(year,month,choiceDay,0,0,0);
            }else {
...
//弹窗各个地方对应时间赋值
function timeChoiceValue(year,month,day,hour,minute,second){
    var box = $(".borain-timeChoice");
    ...
    box.find(".revealBox").find(".secBox span").text(second); 
}
//可选日期点击选中样式以及赋值
...
function setSecond(){
    var box = $(".borain-timeChoice");
    var minAdd = box.find(".revealBox .timeBox").find(".secBox .add");
    var minMinus = box.find(".revealBox .timeBox").find(".secBox .minus");
    minAdd.unbind("click").click(function () {
        var min = parseInt(box.find(".revealBox .timeBox").find(".secBox span").text());
        if(min<59){
            min = min+1;
        }else {
            min = 0;
        ...
        box.find(".revealBox .timeBox").find(".secBox span").text(min);
    });
    minMinus.unbind("click").click(function () {
        var min = parseInt(box.find(".revealBox .timeBox").find(".secBox span").text());
        if(min>0){
...
function notEndTimeChoiceSubmit(obj){
    ...
    var year,month,day,hour,minute,second,val;
...
function hasEndTimeChoiceSubmit(obj,start,end,level,less){
    ...
    var tip = box.find(".choiceBox .tipBox");
    var submitBtn = box.find(".operateBox").find(".submit");
    var year,month,day,hour,minute,second,val,choiceEndVal,choiceStartVal,startDate,endDate;
    submitBtn.unbind("click").click(function () {
        if(obj.is(start)){
            if(end.val()==null||end.val()==""){
                ...
                if(box.hasClass("YM")){
                    ...
                    minute = box.find(".revealBox .timeBox").find(".minBox span").text();
                    second = box.find(".revealBox .timeBox").find(".secBox span").text();
                    choiceStartVal = year+"-"+month+"-"+day+" "+hour+":"+minute+":"+second;
                }
            ...
            if(start.val()==null||start.val()==""){
                ...
                if(box.hasClass("YM")){
                    ...
                    minute = box.find(".revealBox .timeBox").find(".minBox span").text();
                    second = box.find(".revealBox .timeBox").find(".secBox span").text();
                    choiceEndVal = year+"-"+month+"-"+day+" "+hour+":"+minute+":"+second;
                }
